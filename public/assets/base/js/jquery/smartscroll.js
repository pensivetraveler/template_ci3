  ...
  $.smartscroll = function smartscroll(overrides) { // eslint-disable-line no-param-reassign
            ...
            if (scrollAction) {
              if (scrollAction === 'up') {
                if (options.toptotop) {
				  ...
				  var vtotal = docH - (headerH - 10);
                  scrollToPixel(
                    //sections[sectionIndexAtWindowMiddle - 1] - $(window).height()
					  sections[sectionIndexAtWindowMiddle - 1] - vtotal
                    , options.animationSpeed
                  );
				 // console.log(sectionIndexAtWindowMiddle); scroll section idex
                }
                if (options.eventEmitter) {
                  options.eventEmitter.emitEvent('scrollStart', [sectionIndexAtWindowMiddle - 1]);
                }
              } else if (scrollAction === 'down') {
				if(sectionIndexAtWindowMiddle < 7){ // sectin 7 chk
	...
	if (options.mode === 'set') {
      ...
      if (options.ie8) {
        ...
        // Run resizeToVP whenever the window resizes
        $(window).bind('resize', resizeToSeT);
      } else {
        // Use viewport to avoid binding to resize events
		var headerH  = $('#header-wrap').outerHeight();
		var docH = $(window).innerHeight();
