...
class MY_Log extends CI_Log
{
    ...
    public function __construct()
    {
        ...
        $config =& get_config();
        include APPPATH . 'config/extra/sentry_config.php';

        if($config['sentry_log'] === TRUE && $config['sentry_client'] && file_exists($config['sentry_path'])) {
            try {
                $sentry_logging_level = 4 - $config['log_threshold'];
                ...
                $this->_sentry_options = array(
                    'sentry_log_threshold' => $config['sentry_logging_levels'][$sentry_logging_level],
