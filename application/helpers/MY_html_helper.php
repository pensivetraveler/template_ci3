...
function get_help_block_html($data = []) {
    if(empty($data)) return '';

    $selector = $data['tag']??'span';

    // attr
    if(!is_empty($data, 'attr')) {
        $attr = '';
        ...
        // id
        $id = '';
        if(!is_empty($data['attr'], 'id')) $id = '#'.$data['attr']['id'];
        ...
        $selector = $selector.$id.$class.$attr;
    }

    return convert_selector_to_html($selector, true, $data['text']);
}
