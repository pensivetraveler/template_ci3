...
class Format {
    ...
    protected function _from_multipart($input)
    {
        $header = explode('Content-Disposition', $input);
        $header = array_filter($header);
        $header_key = $header[0];
        // 각각의 키-값 쌍으로 분리합니다.
        $parts = explode($header_key, $input);
        // 마지막 요소는 불필요한 공백일 수 있으므로 제거합니다.
        $parts = array_filter($parts);
        // 각각의 쌍을 순회하며 데이터를 추출합니다.
        $data = [];
        foreach ($parts as $part) {
            // Content-Disposition 헤더에서 name 속성의 값을 추출합니다.
            preg_match('/name="([^"]+)"/', $part, $matches);
            $key = $matches[1];
            // 키와 값을 추출하여 데이터 배열에 저장합니다.
            if(strpos($part, 'filename=') !== false) continue;
            preg_match('/Content-Disposition: form-data; name="'.$key.'"\s+(.*?)\n/', $part, $matches);
            $value = $matches[1];
            $data[$key] = $value;
        }
        return $data;
