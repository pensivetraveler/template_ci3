...
class MY_Form_validation extends CI_Form_validation
{
	...
	public function get_error_msg($rule, $field = '', $param = '', $user_errors = [])
	{
		...
		if(element("form_validation_$rule", $this->CI->lang->language)){
			...
			$msg = $this->build_error_msg($line, $field, $param);
		}
		return $msg;
	}
...
//        if( trim((string)$num) == "0" || trim((string)$eng) == "0" || trim((string)$spe) == "0" ) {
    ...
    public function concat($value, $params)
    {
        $exploded = explode('|', $params);
        $fields = explode(',', $exploded[0]);
        $separator = isset($exploded[1])?entity_to_literal($exploded[1]):'';
        $valid = false;

        if(count($fields)) {
            $fields = array_map(function ($item) {
                return $this->_field_data[$item]['postdata'];
            }, $fields);
            $valid = implode($separator, $fields) === $value;
        }
